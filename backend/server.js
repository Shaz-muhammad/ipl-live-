...
function mergeScheduleWithLive(schedule, liveMatches) {
  ...
  const merged = schedule.map((fixture) => {
    ...
    const fixtureDateTime = new Date(
      `${fixture.dateRaw}T${fixture.timeRaw}:00+05:30`,
    );
  ...
  try {
    ...
    async function pollAndEmit() {
      try {
        ...
        console.log(
          `🔴 Live: ${
            mergedMatches.filter((m) => m.status === "live").length
          }, 📅 Upcoming: ${
            mergedMatches.filter((m) => m.status === "upcoming").length
          }, ✅ Finished: ${
            mergedMatches.filter((m) => m.status === "finished").length
          }`,
        );
