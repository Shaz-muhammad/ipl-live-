...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await api.post<ContactApiResponse>("/contact", formData);
      ...
      if (typeof err === "object" && err !== null && "response" in err) {
        ...
        errMsg = apiError.response?.data?.error || errMsg;
      }
      ...
      <main className="flex-1 container mx-auto px-6 py-16 space-y-20">
        <section className="text-center space-y-6 max-w-3xl mx-auto pt-10">
          ...
          >
            ...
            <p className="text-sm md:text-base text-muted-foreground leading-relaxed max-w-2xl mx-auto font-body">
              For match issues, live link concerns, partnership queries, or
              platform support, contact us through the form below or use the
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <section className="space-y-12">
            <div className="space-y-8">
              <h2 className="font-heading text-2xl font-bold text-foreground flex items-center gap-3">
                <Headphones className="h-6 w-6 text-primary" /> Support
                Information
              </h2>
            ...
            <div className="glass-card p-6 border border-primary/20 bg-primary/5 rounded-2xl relative overflow-hidden group">
              ...
              <p className="text-[11px] text-muted-foreground leading-relaxed font-body relative z-10">
                <span className="text-primary font-bold">Pro Tip:</span> Include
                your Match ID or Device Type if reporting a technical glitch for
                faster resolution.
              </p>
            </div>
          </section>
          ...
          <section className="glass-card p-8 md:p-10 border border-border/50 rounded-[32px] relative overflow-hidden">
            ...
            <h2 className="font-heading text-2xl font-bold text-foreground mb-8 relative z-10">
              Send Message
            </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    } rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm`}
                  />
                  {errors.name && (
                ...
                <textarea
                  ...
                  className={`w-full px-4 py-3 bg-secondary/50 border ${
                    errors.message
                      ? "border-destructive/50"
                      : "border-border/50"
                  } rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all text-sm resize-none`}
                />
                {errors.message && (
                  <p className="text-[10px] text-destructive ml-1">
                    {errors.message}
